...
def user_login(request):
    if request.method == 'POST':
        ...
        form = UserLoginForm()
    return render(request, 'counters/login.html', {"form": form})
...
class HomeNews(ListView):
    ...
    """
    model = Counters
    template_name = 'counters/home_news_list.html'
    ...
    paginate_by = 5  # по сколько новостей из списка выводить
    ...
    def get_queryset(self):
        ...
        """
        # return Counters.objects.filter(published=True).select_related('category')
        # user = self.request.user
        # return Counters.objects.filter(user=user)
        return Counters.objects.all()
...
class ViewNews(DetailView):
    model = Counters
    # template_name = 'news/counters_detail.html'
...
#     else:
#         form = NewsForm()
#     return render(request, 'news/add_counter.html', {'form': form})

class CreateNews(LoginRequiredMixin, CreateView):
    form_class = NewsForm
    template_name = 'counters/add_counter.html'
    # login_url = '/admin/'
